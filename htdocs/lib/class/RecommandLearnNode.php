  ...
  */
  class RecommandLearnNode
  {
		...
		*/	
		...
		{
			$result = $this->conDB->prepare("SELECT ".$this->conDB->table("target").".TID,".$this->conDB->table("target").".Mj,".$this->conDB->table("target").".PLj,".$this->conDB->table("belong").".weights,".$this->conDB->table("target").".Fj,".
			...
			while($row=$result->fetch())
			{
				$node+=array("TID"=>$row["TID"],"PLj"=>$row["PLj"],"Mj"=>$row["Mj"],"S" => $row["S"],"Fj"=>$row["Fj"],"MoveTime"=>$row["MoveTime"],"weights"=>$row["weights"],"TLearn_Time"=>$row["TLearn_Time"]);
			}
			return $node;
		}
